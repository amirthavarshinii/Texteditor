			...
			if(isWord(s)) {
				...
				s = s.toLowerCase();
			     for(int i = 0; i < s.length(); i++) {
					char c = s.charAt(i);//Sentence
					if(c=='a'||c=='e'||c=='i'||c=='o'||c=='u'||c=='y') {
						if( c != 'e'	|| (c  == 'e') && ( i != s.length()-1) || count == 0) {
							count = count + 1;
							this.numSyllables = this.numSyllables +1;
						}
					}
						while ( s.charAt(i) =='a'||s.charAt(i) =='e'||s.charAt(i) =='i'||s.charAt(i)=='o'||s.charAt(i)=='u'||s.charAt(i)=='y') {
						   	i++;
						   	if(i >= s.length()-1) {
						   		break ;
						   	}
				...
				this.numSentences = this.numSentences +1;
			}
		}
	if( size-1 >=0 && isWord(tokens.get(size-1) )) {
